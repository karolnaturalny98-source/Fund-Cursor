...
## Problem: pozostałe niespójności globalne – pełny audyt

### Kolory
- `components/home/home-hero.tsx:105-175` – hero nadal buduje tło z `bg-gradient-to-b from-black via-black/60 to-transparent`, `text-white`, `badge` z `border-white/10` i `text-white/80`, co omija surface-tokeny. Efekt to inna „waga” od reszty landingów i duplikacja poziomów kontrastu. Rekomendacja: otoczyć hero `Surface`/`Section` wariantem (`surface-glass`/`surface-muted` + tokenowe cienie) i przenieść gradient do dekoracji (np. pseudo-element) zamiast opierać się na hexach; tekst powinien czerpać z `Text`/`Heading` i tokenów `text-foreground`.
- `components/ui/animated-tooltip.tsx:69-111` – tooltip oraz awatary używają `bg-black`, `border-white`, `text-white`, `shadow-xl`, `bg-gradient-to-r` i `text-xs` z manualnym `font-bold`. Komponent ten jest reużywany w hero, więc te kolory zmieniają wszystkie miejsca detekcji. Powinien opierać się na `Surface`/`Card` z tokenami (`bg-card/80`, `text-muted-foreground`, `border-border/40`, `shadow-soft`), a gradienty przełożyć na akcenty (np. `bg-[var(--surface-ring)]`).
- `app/admin/layout.tsx:23-31` – cały ekran admina ma `bg-black`, podczas gdy `body` i `Section` stosują tokenowe `bg-background` lub `surface-*`. Tło powinniśmy zastąpić warstwą `Surface`/`Section` (np. full-height `Section` z `bg-background/90` i `shadow-premium`), żeby admin panel był częścią tej samej palety i nie wprowadzał surowego czarnego z zewnątrz.
- `components/companies/company-hero-client.tsx:1-15` – hero firmowy używa `bg-card/72 backdrop-blur` plus gradient `bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.05),transparent_50%)]`, co generuje ciemno-granatowe / szare poświaty. Ponieważ komponent renderuje pojedynczy rounded header, ta dekoracja przenosi się na wszystkie strony firm i jest źródłem „podbitego” flasha. Rekomendacja: zastąpić gradient jedynie delikatnym tokenowym halo (`bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.05),transparent_70%)]` lub `bg-[var(--surface-ring)/5%]`) albo usunąć go całkowicie, trzymając tło w `bg-[var(--surface-muted)]` z `border-border`.
- `components/analysis/analysis-layout.tsx:70-90` & `components/Aurora.tsx` – strona ` /analizy/[slug]` dokłada full-screen aurorę z własnym shaderem (kolory w `components/Aurora.tsx`), co jest jedynym miejscem nadającym tło w tej tonacji i trudno go wyłączyć w konfiguracji. Jeśli aurora jest zbędna lub ma zostać wycofana, należy usunąć import oraz `<Aurora />` z layoutu i zamiast animowanego shaderu użyć `Surface`/`Section` z tokenowym `bg-card/72` plus ewentualnymi gradientami akcentowymi (np. `bg-gradient-to-br from-primary/5 to-card/10`). Jeżeli aurorę potrzebujemy nadal, trzeba ją osadzić wewnątrz `Surface` i wyraźnie oznaczyć jako ustaloną dekorację, nie podstawowe tło.

### Border
- `components/home/home-hero.tsx:125-162` – hero badge + CTA stosują `border-white/10` i `border-white/15`, co rozbija koncepcję `border-border`. Zamiast tego wpisy powinny używać `border-border/60` (albo `border-primary/40` dla akcentu) i zostawić `border-white/*` tylko tam, gdzie naprawdę zależy nam na bardzo jasnej ramce (której w tej sekcji nie ma). Dzięki temu mogą korzystać z tej samej hierarchii co inne cards.
- `components/layout/site-navbar.tsx:99-133` – mobilny „Konto” w menu używa `border-white/20`, podczas gdy reszta nawigacji przeszła na `border-border`. Pozostawienie białego borde` powoduje przełamanie tonalne w mobilnym panelu. Wymagane: zmienić na `border-border/40`, żeby utrzymać spójną siatkę borderów i ułatwić tworzenie wariantów `NavbarButton`.

### Surface / cienie
- `components/layout/site-header.tsx:32-80` – górna sekcja bazy i aktywny link korzystają z niestandardowych `shadow-[0_30px_120px_rgba(0,0,0,0.45)]`, `shadow-[0_0_0_1px_rgba(255,255,255,0.05)]`, gradientowego `bg-gradient-to-b` i „Zaloguj” button z `shadow-[0_10px_40px_rgba(45,212,191,0.45)]`. To tworzy kilka różnych głębokości i występuje poza systemem `shadow-soft`/`shadow-premium`. Wersja docelowa: zastąpić te cienie tokenami albo nowym wariantem Surface (np. `shadow-soft` + `backdrop-blur`) oraz przenieść gradientowe CTA do `Button variant="premium"` żeby utrzymać jednorodność warstw.

### Spacing / utility CSS
- `components/home/home-hero.tsx:105-175` – choć hero osadzono w `Section`, wewnętrzny układ nie korzysta z `stack` (domyślnie `none`) i dodaje `gap-12`, `px-4`, `py-[clamp(...)]` oraz `space-y-4` z własnymi wartościami. Powoduje to, że hero nie dzieli fluid-stack z pozostałymi sekcjami. Rekomendacja: zadeklarować `Section stack="xl"` lub `stack="lg"` i zastąpić ręczne `gap`s komponentami `Surface`/`Stack`, by spacing pozostał w gestii `Section`.
- `components/companies/compare-bar.tsx:20-40` – compare bar korzysta z `fluid-copy` i `fluid-caption` plus 3 różne `gap`/`p` z clampami, mimo że mamy `Text` i spacing `surface-pad-*`. Każda zmiana w `fluid-*` w `globals.css` generuje potrzebę poprawki tutaj. Lepiej opakować CTA w `SurfaceCard`/`Section` i użyć `Text` z `tone="muted"`/`weight` zamiast legacy utili, co pozwoli w przyszłości usunąć `fluid-copy`/`fluid-caption`.

### Typografia
- `components/home/home-hero.tsx:130-150` – hero piece używa `<p className="max-w-2xl text-base text-white/70">` i przekazuje `className="text-white"` do `TypewriterEffectSmooth`. Tego typu manualne klasy typograficzne omijają `Text`/`Heading`, przez co kontrast i responsywność nie są centralnie kontrolowane. Powinno się użyć `Text variant="lead"`/`Heading level={1}` z `tone`, żeby typografia korzystała z tokenów (np. `fluid-h1`, `fluid-lead`) i masa z kontrolowanych `className`.
- `components/companies/compare-bar.tsx:25-38` – treści `Wybrane do porownania` oraz instrukcje używają `<p>` i `<span>` z `font-semibold text-foreground`, `text-foreground/80`, `break-all font-medium text-foreground`, a w akapitach `fluid-caption`. To omija `Text` i generuje kolejnych „manualnych” typów (np. `text-foreground/80`). Rekomenduję `Text`/`Heading` z `tone="muted"`/`weight="semibold"` i `Text variant="caption"`; `fluid-caption` można usunąć, skracając `globals.css`.

### Semantyka
- `components/companies/compare-bar.tsx:41-55` – CTA „Porownaj” to `<Link>` stylizowany przez `buttonVariants`, ale bez `role`/`type`/`asChild`. W konsekwencji narusza semantykę przycisku i nie pozwala w pełni wykorzystać fokus/disabled states z `Button`. Wystarczy użyć `<Button asChild size="sm">` i wstawić `<Link>` jako `children`, żeby zachować styl, dostępność i aria-disabled.
- `components/home/home-hero.tsx:147-162` – hero CTA „Zobacz ranking” (via `HoverBorderGradient`) i badge „Program cashback” mają wygląd „button-like”, ale są `Link`/`Badge` bez dedykowanej semantyki (brak `role`, focus ring zależy od `Link`). Powinny korzystać z `Button`/`Badge variant="hero"` z `asChild`, co daje dostępność i pozwala reużywać gradientowe tła w kontrolowany sposób.

## Plan naprawy – Iteracja końcowa spójności (bez zmian w designie)
- **Co naprawić:** hero i tooltipy przenoszą się na surface-tokeny, nav/mobilny border wymaga `border-border`, compare bar musi korzystać z `Text` i `Button asChild`, a admin layout oraz headery nie mogą trzymać surowych `bg-black`/`shadow-*`.
- **Co naprawić (dodatkowo):** zmodyfikować `CompanyHeroClient` by usunąć ciemne gradienty/podzielenia oraz wyjąć komponent `Aurora` z `analysis-layout`, zastępując go tokenowym `Surface` lub wyróżnionym CTA.
- **W jakiej kolejności:** 1) „bazowe” tła (hero hero/tooltip/admin) – bo definiują paletę; 2) nawigacje/mobile + nowy surface/shadow w `SiteHeader`; 3) compare bar spacing + typography; 4) semantyka CTA (linki/buttony).
- **Które elementy mogą być ryzykowne:** `SiteHeader` (zależne od stanu loginu/admin) i `CompareBar` (dynamiczny render przy wyborze firm) będą wymagały sprawdzenia focus/keyboard oraz zachowanie `clear/compare` logicznych ścieżek.
- **Co można zautomatyzować:** `rg --line-number 'border-white'`, `rg 'bg-\\[#0'`, `rg 'shadow-\\['` i `rg 'fluid-(copy|caption|pill)'` szybko znajdą pozostałości; te rezultaty mogą być wykorzystane jako checklisty do ręcznych poprawek.
- **Co wymaga manualnej decyzji:** Czy hero dalej potrzebuje gradientu (jeśli tak, zaprojektować go jako dekorację z `Surface`/`Section`), jakie dokładnie `shadow` w `SiteHeader` zostaną przekopiowane do `shadow-soft`/`shadow-premium`, oraz które `HoverBorderGradient` CTA zostaną zastąpione wariantem `Button`.
