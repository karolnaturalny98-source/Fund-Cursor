...
export function SiteHeader() {
  ...
  return (
    <Navbar className="pointer-events-none !fixed inset-x-0 !top-4 z-50 flex justify-center px-4">
      <NavBody className="pointer-events-auto hidden w-full max-w-6xl items-center gap-4 rounded-full border border-border/40 bg-[var(--surface-muted)]/70 px-4 py-2 text-sm shadow-[0_30px_120px_rgba(0,0,0,0.45)] backdrop-blur-xl lg:flex">
        <Link
          className="flex items-center gap-3 rounded-full border border-border/40 px-3 py-2 text-xs font-semibold uppercase tracking-[0.4em] text-white/80"
          href="/"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            variant="outline"
            className="rounded-full border border-border/40 bg-card/72 text-foreground transition hover:bg-[var(--surface-muted)]/60 hover:text-white"
            onClick={open}
          >
            Panel
          </Button>
      ...
      <MobileNav className="pointer-events-auto lg:hidden">
        <MobileNavHeader className="w-full rounded-[28px] border border-border/40 bg-[var(--surface-muted)]/80 px-4 py-3 text-white">
          ...
          <div className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border/40">
            <MobileNavToggle
              isOpen={mobileOpen}
        ...
        <MobileNavMenu
          ...
          className="mx-auto mt-3 w-full max-w-sm border border-border/40 bg-[var(--surface-muted)]/90 text-white"
        >
          <nav className="flex w-full flex-col gap-2">
            {navigation.map((item) => (
              <Link
                ...
                className={cn(
                  "rounded-2xl border border-border/40 px-4 py-3 text-sm text-white/70 transition",
                  pathname === item.href || pathname.startsWith(`${item.href}/`)
                    ? "bg-[var(--surface-muted)]/60 text-white shadow-[0_0_0_1px_rgba(255,255,255,0.05)]"
                    : "hover:text-white",
                )}
              >
                {item.label}
          ...
          <div className="flex w-full flex-col gap-3 rounded-2xl border border-border/40 bg-card/72 p-4 text-sm text-white/80">
            <CurrencySwitcher />
              <Button
                variant="outline"
                className="w-full rounded-full border border-border/40 bg-card/72 text-foreground transition hover:bg-[var(--surface-muted)]/60 hover:text-white"
                onClick={() => {
                  open();
                  setMobileOpen(false);
                }}
            ...
            <SignedIn>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-border/40 px-4 py-2">
                <span>Konto</span>
