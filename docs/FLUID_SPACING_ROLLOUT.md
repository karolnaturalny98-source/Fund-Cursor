...
| Public – Analysis | `app/(site)/analizy/page.tsx` + `components/analysis/*` | Hero + benefit tiles rely on `space-y` and card-specific padding | In Progress | Hero + CompanySelector + ReviewSentiment używają `fluid-*`; reszta komponentów analizy do migracji |
...
| Public – Blog hub | `components/blog/blog-categories-tabs.tsx`, `blog-post-card.tsx` | Tabs and cards define `space-y` + per-card padding | Done | Tabs i karty korzystają z `fluid-pill`, `fluid-stack`, alerty z `p-4` |
...
| Shop | `components/shop/*` (filters, badges, pill buttons) | Remaining controls still hard-code `px`/`py` clamp values | In Progress | Karty firm + formularz zakupowy korzystają z `fluid-card-pad-*`; plan cards do migracji |
