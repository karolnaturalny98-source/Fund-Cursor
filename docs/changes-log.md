...
- Przywrócono standardowy zestaw zmiennych kolorystycznych shadcn (light/dark) wraz z tokenami sidebara i chartów, więc motywy eksportowane z registry/tweakcn można nakładać 1:1, a niestandardowe efekty (`surface-glass`, `shadow-*`) korzystają z tych samych bazowych wartości.
- Nawigacja w `SiteHeader` korzysta z `fluid-pill` i standardowych spacingów Tailwinda zamiast ręcznych `px-[clamp(...)]`, dzięki czemu header dziedziczy wygląd domyślnych komponentów shadcn na desktopie i mobile.
- Zakładki w `components/blog/blog-categories-tabs.tsx` oraz karty `blog-post-card.tsx` korzystają z `fluid-pill`, `fluid-stack-*` i standardowych paddingów (`p-4`/`p-5`) zamiast ręcznych `clamp`; w trackerze „Public – Blog hub” oznaczono jako Done.
- W sklepie (`shop-company-cards.tsx`, `shop-purchase-form.tsx`) statystyki, szybkie filtry, karty firm i formularz zakupu korzystają z `fluid-pill`, `fluid-card-pad-*`, `fluid-input-icon`, `fluid-avatar-md` i `fluid-icon-*`, więc większość modułu sklepu jest na docelowych utilach (tracker „Shop” pozostaje In Progress – zostały plan cards/pozostałe komponenty).
- Sekcja hero sklepu (`shop-page-client.tsx`) ma statystyki na `fluid-card-pad-md` + `fluid-h2`, a `TabsContent` używa prostych odstępów zamiast clampów, więc wygląd kart odpowiada landingowi.
- `/analizy` (landing) oraz `CompanySelector`, `AnalysisLayout` i `ReviewSentiment` zostały przepisane na nowe utilsy (`fluid-pill`, `fluid-card-pad-*`, `fluid-avatar-*`, `fluid-stack-2xl/3xl`), dzięki czemu hero, selektor firm i karty analityczne korzystają z tych samych tokenów co strona główna; kolejne komponenty (np. trading conditions) pozostają do migracji.
