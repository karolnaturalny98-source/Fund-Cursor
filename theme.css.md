# theme.css.md
Theme kolorów i tokenów dla frontendu v2

Poniższy blok CSS z `:root { ... }` pochodzi z TweakCN.  
Agent ma go traktować jako **jedyne źródło prawdy** dla kolorów i bazowych tokenów UI w v2.

## Zasady

- **Nie zmieniać wartości** wygenerowanych przez TweakCN (chyba że JA to potem zmodyfikuję).
- Używać kolorów WYŁĄCZNIE w formie `hsl(var(--nazwa-zmiennej))`.
- W `tailwind.config` mapować kolory do tych zmiennych (np. `background`, `foreground`, `primary`, `accent`, `border`, itp.).
- Wszystkie komponenty v2 (shadcn/ui, ReactBits, Aceternity) mają używać tych samych tokenów.
